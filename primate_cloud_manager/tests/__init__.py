# -*- coding: utf-8 -*-
from . import test_crypto
from . import test_aws_base
from . import test_operation_log
from . import test_account
from . import test_services_phase2
from . import test_sync_phase2
from . import test_models_phase2
from . import test_phase3
from . import test_phase4
from . import test_phase5
from . import test_phase6
from . import test_phase7
from . import test_phase8
from . import test_phase85
from . import test_phase9
from . import test_pcm_app
from . import test_integration_moto
